...
function addJob() {
    ...
    if (existingJobIndex !== -1) {
        alert(`El trabajo con el nombre "${jobName}" ya existe. Será eliminado.`);
        releaseJob(jobName);
        return;
    }
    ...
    if (!assigned) {
        let totalFreeMemory = memoryBlocks.filter(block => !block.occupied).reduce((sum, block) => sum + block.size, 0);
        if (totalFreeMemory >= jobSizeInKB) {
            alert("Fragmentación detectada. Compactando memoria...");
            compactMemory();
            addJob(); // Reintentar agregar el trabajo tras compactar
        } else {
...
function assignWaitingJob() {
    for (let i = 0; i < jobQueue.length; i++) {
        ...
        let totalFreeMemory = memoryBlocks.filter(block => !block.occupied).reduce((sum, block) => sum + block.size, 0);
        if (totalFreeMemory >= waitingJob.size) {
            alert("Fragmentación detectada. Compactando memoria para trabajos en espera...");
            compactMemory();
            assignWaitingJob(); // Reintentar tras compactar
            return;
        }
    }
}
...
function compactMemory() {
    ...
    for (let block of memoryBlocks) {
        if (block.occupied) {
            compactedBlocks.push({ ...block, start });
            start += block.size;
        }
    }

    let freeMemory = memoryBlocks.filter(block => !block.occupied).reduce((sum, block) => sum + block.size, 0);
    if (freeMemory > 0) {
        compactedBlocks.push({ start, size: freeMemory, occupied: false });
    }
...
function compactAdjacentFreeBlocks() {
